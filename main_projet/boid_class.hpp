...
void spawn_boids_repartition_exp(const std::vector<Boid> &boids_tab);
...
public:
  ...
  Boid(float sepRad = 0.1, float cohRad = 0.1, float aliRad = 0.3)
      : pos(loi_uniforme(-1, 1), loi_uniforme(-1, 1)),
  ...
  void separation(const std::vector<Boid> &boids_tab, float separationForce) {
    ...
    if (distance_mini < separationRadius) {
      glm::vec2 modifier = normalize_to_vit(this_minus_other_pos, vit) *
