...
int main() {
  ...
  const std::vector<glimac::ShapeVertex> sphereVertices =
      glimac::sphere_vertices(radius, Nlat, Nlong);
  ...
  glBufferData(GL_ARRAY_BUFFER,
               sphereVertices.size() * sizeof(glimac::ShapeVertex),
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, earthTex.width(), earthTex.height(),
               0, GL_RGBA, GL_UNSIGNED_BYTE, earthTex.data());
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, moonTex.width(), moonTex.height(), 0,
               GL_RGBA, GL_UNSIGNED_BYTE, moonTex.data());
  glBindTexture(GL_TEXTURE_2D, 0);
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, cloudTex.width(), cloudTex.height(),
               0, GL_RGBA, GL_UNSIGNED_BYTE, cloudTex.data());
  glBindTexture(GL_TEXTURE_2D, 0);
  ...
  for (int i = 0; i < nbOfMoons; i++) {
    ...
    std::cout << "[" << i << "] x: " << moons[i].x << ", y: " << moons[i].y
              << ", z: " << moons[i].z << std::endl;
  }
  ...
  ctx.update = [&]() {
    ...
    const glm::mat4 globalProjMatrix =
        glm::perspective(glm::radians(70.f), ctx.aspect_ratio(), .1f, 100.f);
    ...
    earthMVMatrix =
        glm::rotate(earthMVMatrix, glm::radians(180.0f), {1.f, 0.f, 1.f});
    ...
    glUniformMatrix4fv(earthProgram.uMVPMatrix, 1, GL_FALSE,
                       glm::value_ptr(globalProjMatrix * earthMVMatrix));
    ...
    for (int i = 0; i < moons.size(); i++) {
      ...
      glm::mat4 moonMVMatrix =
          glm::rotate(globalMVMatrix, ctx.time(), {0.f, 1.f, 0.f});
      ...
      glUniformMatrix4fv(moonProgram.uMVPMatrix, 1, GL_FALSE,
                         glm::value_ptr(globalProjMatrix * moonMVMatrix));
